...
func TestHandlerCheckLanguage(t *testing.T) {
	...
	scope, err := c.NewScope(&ModelI18n{})
	assert.NoError(t, err)
	...
	scope.Value = &ModelI18n{ID: 1, Lang: "pl"}
	...
	rw = httptest.NewRecorder()
	scope.Value = &ModelI18n{ID: 3, Lang: "invalid_lang"}
	langTag, ok = h.CheckValueLanguage(scope, rw)
	...
	rw = httptest.NewRecorder()
	scope.Value = &ModelI18n{ID: 3, Lang: language.Arabic.String()}
	langTag, ok = h.CheckValueLanguage(scope, rw)
	...
	err = c.PrecomputeModels(&Model{})
	assert.NoError(t, err)

	scope, err = c.NewScope(&Model{})
	assert.NoError(t, err)

	scope.Value = &Model{ID: 1, Name: "Some name"}
